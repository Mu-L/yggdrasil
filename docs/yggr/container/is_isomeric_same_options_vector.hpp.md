...
`is_isomeric_same_options_vector.hpp` 是 yggdrasil 项目中用于判断带有选项参数的 vector 类型在“同构异参”意义下是否相同的类型萃取工具。该文件主要解决 Boost 1.70 及以后版本中 `boost::container::vector` 模板参数和 options 参数变化导致的类型兼容性问题，便于泛型编程时安全判断不同 vector 类型（含 options）的等价性。
...
- 在 `yggr::container::detail` 命名空间下实现 `_is_isomeric_same_options_vector_detail` 模板结构体，递归判断两个带 options 的 vector 类型的模板参数、值类型、分配器类型、options 类型等是否“同构异参”。
  - 对于模板参数数量不同、类型不同等情况，返回 `false_`。
  - 对于模板参数数量为 2 的情况，复用基础 vector 的同构判断。
  - 对于模板参数数量为 3 的情况，判断第二、三模板参数是否不同，值类型、分配器类型、options 类型是否一致。
- 提供 `is_isomeric_same_options_vector<VtL, VtR>` 类型萃取，判断两个带 options 的 vector 类型是否“同构异参”。
- 提供 `is_same_or_isomeric_same_options_vector<VtL, VtR>` 类型萃取，判断两个类型是否完全相同或“同构异参”。
