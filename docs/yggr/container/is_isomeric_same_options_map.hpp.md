...
`is_isomeric_same_options_map.hpp` 是 yggdrasil 项目中用于判断带有选项参数的 map/multimap 类型在“同构异参”意义下是否相同的类型萃取工具。该文件主要解决 Boost 1.70 及以后版本中 `boost::container::map` 模板参数和 options 参数变化导致的类型兼容性问题，便于泛型编程时安全判断不同 map 类型（含 options）的等价性。
...
- 在 `yggr::container::detail` 命名空间下实现 `_is_isomeric_same_options_map_detail` 模板结构体，递归判断两个带 options 的 map 类型的模板参数、值类型、分配器类型、key 比较器类型、options 类型等是否“同构异参”。
...
- 提供 `is_isomeric_same_options_map<MapL, MapR>` 类型萃取，判断两个带 options 的 map 类型是否“同构异参”。
- 提供 `is_same_or_isomeric_same_options_map<MapL, MapR>` 类型萃取，判断两个类型是否完全相同或“同构异参”。
