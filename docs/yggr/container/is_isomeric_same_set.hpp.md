...
`is_isomeric_same_set.hpp` 是 yggdrasil 项目中用于判断 set/multiset 类型在“同构异参”意义下是否相同的类型萃取工具。该文件主要解决 Boost 1.70 及以后版本中 `boost::container::set` 模板参数变化导致的类型兼容性问题，便于泛型编程时安全判断不同 set 类型的等价性。
...
- 在 `yggr::container::detail` 命名空间下实现 `_is_isomeric_same_set_detail` 模板结构体，递归判断两个 set 类型的模板参数、值类型、分配器类型、key 比较器类型等是否“同构异参”。
  - 对于模板参数数量不同、类型不同等情况，返回 `false_`。
  - 对于模板参数数量为 3 的情况，判断第三模板参数（如 void）不同，值类型、分配器类型、key 比较器类型一致。
- 提供 `is_isomeric_same_set<SetL, SetR>` 类型萃取，判断两个 set 类型是否“同构异参”。
- 提供 `is_same_or_isomeric_same_set<SetL, SetR>` 类型萃取，判断两个类型是否完全相同或“同构异参”。
