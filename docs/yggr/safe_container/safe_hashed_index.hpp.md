...
# 文件: safe_hashed_index.hpp 文档

# 文件简介

safe_hashed_index.hpp 是 yggdrasil 项目中安全容器模块的一个头文件，主要实现了多索引哈希表（multi_index hashed index）的线程安全优化版本。该文件依赖 Boost、多索引容器和 yggdrasil 自身的基础设施，通过模板和宏实现高性能、线程安全的哈希索引操作。
